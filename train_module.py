...
def train_env_ours(epoch, net, train_loader, args, training_opt, variance_opt, loss_function, optimizer, warmup_scheduler,avg_precision,avg_loss, exp_name):
    start = time.time()
    ...
    for batch_index, data_env in enumerate(train_loader[0]):
        ...
        if batch_index % training_opt['print_batch'] == 0:
            ...
            with io.open('results_txt/' +  exp_name+ '/' + '-train-batch' + '.txt', 'a', encoding='utf-8') as file:
                file.write('Training Epoch: {epoch} [{trained_samples}/{total_samples}]\tTrain_Loss: {:0.3f}\tNll_Loss: {:0.3f}\tPenalty: {:.2e}'
                  '\tPenalty_W: {:.1f}\tErm_Loss: {:.3f}\tSp_Loss: {:.3f}\tLR: {:0.6f}\tAcc: {:0.4f}\n'.format(
                    loss.item(),
                    train_nll.item(),
                    loss_penalty.item(),
                    penalty_weight,
    ...
    with io.open('results_txt/' +  exp_name+ '/' + '-train' + '.txt', 'a', encoding='utf-8') as file:
        file.write('epoch {} training time consumed: {:.2f}s \t Train Acc: {:.4f}\tTrain_Loss: {:0.3f}\n'.format(epoch, finish - start, train_acc_all, np.array(train_loss).mean()))
    avg_precision.append(train_acc_all)
    avg_loss.append(np.array(train_loss).mean())
    #ipdb.set_trace()
    return train_acc_all



def auto_split(ref_model, variance_opt, pre_train_loader, pre_optimizer, pre_schedule, soft_split_all,exp_name,config):
    print(soft_split_all)
    ...
    for epoch in range(100):
        ...
        for data in pre_train_loader:
            ...
            # ************** banlance loss ****************#
            dif_num = []
            s_num = torch.softmax(soft_split_all, 1)
            # ******new balance loss******
            sum_idx = (torch.sum(s_num, 0))
            for e_idx in range(len(sum_idx)):
                dif_num.append(sum_idx[e_idx] - sum_idx[e_idx - 1])
                dif_num[e_idx] = abs(dif_num[e_idx])
            ban_loss = sum(dif_num) / len(dif_num)

            max_split = torch.zeros_like(s_num).scatter_(1, torch.argmax(s_num, 1).unsqueeze(1), 1)
            mid_num = torch.sum(max_split, 0)

            print(ban_loss)
            print(mid_num)
            with io.open('results_txt/' + exp_name + '/' + '-bal_loss' + '.txt', 'a', encoding='utf-8') as file:
                file.write('epoch:' + str(epoch) + '\n')
                file.write(str(sum_idx))
                file.write('\n')
            ...
            if 'lb' in variance_opt and variance_opt['lb'] == True:
                risk_all = irm_risk_final + reg + ban_loss
            else:
        ...
        if epoch == 0:
            ...
            with io.open('results_txt/' +  exp_name+ '/' + '-auto_split' + '.txt', 'a', encoding='utf-8') as file:
                file.write("Initial_Irm_Risk: %.2f" % (avg_irm_risk))
        ...
        with io.open('results_txt/' +  exp_name+ '/' + '-auto_split' + '.txt', 'a', encoding='utf-8') as file:
            file.write('\rInitializing Env [%d/%d]  Loss: %.2f  IRM_Risk: %.2f  Reg: %.2f  Ban: %.2f  Cnt: %d  Lr: %.2f'
              %(epoch, 100, avg_risk, avg_irm_risk, sum(reg_list)/len(reg_list), sum(ban_list)/len(ban_list), cnt, pre_optimizer.param_groups[0]['lr']))



        if epoch > 40 and cnt > 5 or epoch == 99: #debug
...
def refine_split(bias_optimizer, bias_schedule, bias_classifier, classifier_train_loader, feature_extractor,exp_name):
    ...
    for epoch in range(100):
        ...
        for data in classifier_train_loader:
            ...
            with io.open('results_txt/' +  exp_name+ '/' + '-refine_split' + '.txt', 'a', encoding='utf-8') as file:
                file.write('\rOptimizing [%d/%d]  Loss: %.2f  Acc: %.2f  Lr: %.3f'  
                           % (epoch, 100, bias_loss.item(), train_acc, bias_optimizer.param_groups[0]['lr']))
